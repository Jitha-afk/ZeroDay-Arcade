...
import { Award, TrendingUp, Clock, Shield, CheckCircle, XCircle, AlertTriangle } from "lucide-react";
...
interface DebriefPanelProps {
  ...
  microsoftRecommendations: Array<{
    solution: string;
    description: string;
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Session Analysis & Microsoft Security Solutions Recommendations
          </p>
        </div>

        {/* Player Performance Summary */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card data-testid="card-player-score" className={`border ${performance.color.replace('text-', 'border-')}`}>
            <CardContent className="p-6 text-center">
              ...
              <Badge className={`mt-2 ${performance.bgColor} ${performance.color} border-transparent`}>
                {performance.level}
              </Badge>
            </CardContent>
          </Card>

          <Card data-testid="card-team-rank">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                out of {teamStats.length} players
              </div>
            </CardContent>
          </Card>
          ...
          <Card data-testid="card-response-time">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {playerStats.decisions} decisions made
              </div>
            </CardContent>
          </Card>
          ...
          <Card data-testid="card-accuracy">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {playerStats.correctDecisions}/{playerStats.decisions} correct
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card data-testid="card-team-performance">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge variant="outline" className="font-mono">{teamAverage} points</Badge>
              </div>
              ...
              <div className="space-y-3">
                {teamStats.map((player, index) => (
                  <div
                    key={player.name}
                    data-testid={`team-player-${index}`}
                    className={`flex items-center justify-between p-3 rounded-lg border ${
                      player.name === playerStats.name ? 'border-primary bg-primary/5' : 'border-border'
                    }`}
                  >
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {formatDuration(player.responseTime)} avg
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <Card data-testid="card-microsoft-solutions">
            <CardHeader>
              <CardTitle className="font-mono text-blue-500">MICROSOFT_SOLUTIONS</CardTitle>
              <p className="text-sm text-muted-foreground">
                How Microsoft security products could have prevented this breach
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              {microsoftRecommendations.map((rec, index) => (
                <div
                  ...
                  className="p-4 border rounded-lg bg-blue-500/5 border-blue-500/20"
                >
                  <div className="flex items-start justify-between mb-2">
                    <h4 className="font-semibold text-blue-600">{rec.solution}</h4>
                    <Badge variant="outline" className="text-blue-600 border-blue-600">
                      {rec.preventionScore}% prevention
                    </Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">{rec.description}</p>
                  <Progress
                    value={rec.preventionScore}
                    className="mt-2 h-2"
                  />
                </div>
              ))}
